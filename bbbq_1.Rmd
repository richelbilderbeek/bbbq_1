...
f_myco <- rnorm(n = n, mean = runif(n = 1, min = 0.03, max = 0.1), sd = 0.01)
...
df <- tibble::tibble(
  ...
  haplotype = rep(get_haplotypes(), time = 3),
...
ggplot(df, aes(x = haplotype, y = f, fill = proteome)) + 
  ...
  labs(
    ...
    caption = paste0(
      "Straight line: average in humans: ", formatC(100.0 * mean(f_human), digits = 2),"%. \n",
      "Dashed line: average in COVID-19: ", formatC(100.0 * mean(f_covid), digits = 2),"%. \n",
      "Dotted line: average in Mycoplasma: ", formatC(100.0 * mean(f_myco), digits = 2),"%."  
    )
